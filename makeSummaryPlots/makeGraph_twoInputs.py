...
def ParseOption():
    ...
    parser.add_argument('--intxt2', dest='intxt2', type=str, help='')
...
data2 = [line.strip() for line in open(args.intxt2, 'r')]

for i in range(1, len(data)):

    dataperline = data[i].split(' ')
    dataperline2 = data2[i].split(' ')

    sigma_fit.append(float(dataperline[0]))
    sigma_fitErr.append(float(dataperline[1]))
    sigma_fit2.append(float(dataperline2[0]))
    sigma_fitErr2.append(float(dataperline2[1]))

    sigma_pred.append(float(dataperline[3]))
    if args.doREFIT:
       sigma_pred_corr.append(float(dataperline[4]))
    else:
       sigma_pred_corr.append(float(dataperline2[3]))


y, y2, x1, x2 = array('f'), array('f'), array('f'), array('f')
xErr,yErr,yErr2 = array('f'), array('f'), array('f')

for i in range(len(sigma_fit)):
    y.append(sigma_fit[i])
    yErr.append(sigma_fitErr[i])
    y2.append(sigma_fit2[i])
    yErr2.append(sigma_fitErr2[i])
...
gr2 = TGraphErrors(len(y),x2,y2,xErr,yErr2)
...
if not args.doREFIT:
   legend.AddEntry(gr1, 'data', 'p')
   legend.AddEntry(gr2, 'mc', 'p')
