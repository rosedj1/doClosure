...
double MySelector::ApplyCorr(double pT, double eta, double pTErr, int ecalDriven) {
 ...
 if (ecalDriven) {
    ...
    if (abs(eta) < 1 && pTErr/pT > 0.03 ) scale = 1.187;//1.224;//1.187;
    if (abs(eta) >= 1 && pTErr/pT < 0.07 ) scale = pTCorr(pT,eta,1); // LUT_2 is for |eta| > 1 && pTErr/pT < 0.06
    if (abs(eta) >= 1 && pTErr/pT > 0.07 ) scale = 0.815;//0.786;//0.815;
...
//    newBranch = fReader->Branch("massZErr_corr",&massZErr_corr,"massZErr_corr/F");
      fReader.Print();
...
//   if (nEvents > 100) return kTRUE;
   ...
   massZErr_corr = massZErr_cal_corr;
   t3->Fill();
...
//   fChain->Write("",TObject::kOverwrite); // save only the new version of
   f->cd(); t3->Write();   
}
